...
def count_change(total):
    ...
    def largest_po2_in(x):
        assert x > 0, 'x cannot be smaller than 1'

        if x == 1:
            return 0
        else:
            return 1 + largest_po2_in(x // 2)


    def helper(total, maximum):
        if total == 0:
            return 1
            ...
            return helper(total - maximum, maximum) + helper(total, maximum / 2)
    
    return helper(total, pow(2, largest_po2_in(total)))
