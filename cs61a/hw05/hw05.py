...
def make_withdraw(balance, password):
    ...
    "*** YOUR CODE HERE ***"
    lst = []
    def helper(amount, typed_password):
        nonlocal balance
        nonlocal lst
        if len(lst) < 3:
            if typed_password == password and amount <= balance:
                balance = balance - amount
                return balance
            elif typed_password == password and amount > balance:
                return 'Insufficient funds'
            else:
                lst.append(typed_password)
...
def repeated(t, k):
    ...
    count = 0
    for elem in t:
        if count < k and elem == var:
            count += 1
            var = elem
            if count == k:
                return var
        elif count < k and elem != var:
            count = 1
            var = elem
...
def merge(incr_a, incr_b):
    ...
    while True:
        ...
        elif next_a is not None and next_b is None:
            yield next_a
            next_a = next(iter_a, None)
        else:
            break
