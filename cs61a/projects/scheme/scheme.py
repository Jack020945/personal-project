...
def scheme_eval(expr, env, _=None): # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        operands = expr.rest.map(lambda _ : scheme_eval(_, env = env))
...
class BuiltinProcedure(Procedure):
    ...
    def apply(self, args, env):
        ...
        try:
            ...
            raise SchemeError('wrong number of arguments for {0}'.format(self.name))
        # END PROBLEM 3
    ...
    if scheme_symbolp(target):
        ...
        def check_formals(formals):
            if formals.rest is nil:
                return scheme_symbolp(formals.first)
            elif scheme_symbolp(formals.first):
                return True and check_formals(formals.rest)
            return False

        if target.rest is nil or check_formals(target.rest):
...
def do_and_form(expressions, env):
    ...
    elif isinstance(expressions.first, Pair) and expressions.first.first == 'print':
...
def quote_args(args):
            if args is nil:
                ...
                return Pair(Pair('quote', Pair(args.first, nil)), quote_args(args.rest))
