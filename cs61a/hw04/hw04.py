...
def balanced(m):
    ...
    if is_planet(m):
        ...
        rihgt_length = length(right_arm)
        if left_length * left_weight != rihgt_length * right_weight:
            return False
        else:
...
def replace_leaf(t, find_value, replace_value):
    ...
    if is_leaf(t) and label(t) == find_value:
        ...
        lst = []
        for b in branches(t):
            lst.append(replace_leaf(b,find_value,replace_value))
...
def has_path(t, phrase):
    ...
    if label(t) == phrase[0]:
        b_bool = False
        for b in branches(t):
            b_bool = b_bool or has_path(b, phrase[1:])
        return b_bool
