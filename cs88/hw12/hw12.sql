...
CREATE TABLE long_album as
  select title from albums as a,tracks as b where a.albumId = b.albumId and b.milliseconds > 360000 ;
...
CREATE TABLE album_count as
  select name, COUNT(*) from artists as a, albums as b where a.artistID = b.artistID group by name;


-- Question 8
CREATE TABLE busiest_artists as
  select artists.name , count(tracks.albumId)  FROM artists inner join albums on albums.artistId =artists.artistId inner join tracks On tracks.albumId = albums.albumId group by artists.name order by artists.name asc;
