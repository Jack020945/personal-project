...
class Arr88():
    ...
    def __add__(self, arr88):
        ...
        for i in range(len(self._values)):
            total = self.item(i) + arr88.item(i)
            new_lst.append(total)
        return Arr88(new_lst)
    ...
    def apply(self, func):
        ...
        Arr88([1, 2, 3])
        """
        new_lst = self._values.copy()
        for i in range(len(new_lst)):
            new_lst[i]= func(new_lst[i])
        return Arr88(new_lst)
...
class VendingMachine:
    ...
    def vend(self):
        if self.stock == 0:
            ...
            if self.balance > self.price:
                returned_money = self.balance - self.price
                self.balance = 0
                self.stock = self.stock - 1
                return 'Here is your {} and '.format(self.product) + '${} change.'.format(returned_money)
                ...
                needed_money = self.price - self.balance
                return'You must deposit ${} more.'.format(needed_money)
    ...
    def restock(self, number):
        ...
        return 'Current {} stock: '.format(self.product) + '{}'.format(self.stock)
