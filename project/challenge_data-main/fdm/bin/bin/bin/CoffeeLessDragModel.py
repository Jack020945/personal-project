...
def cylinderdrag(cyl_len, cyl_dia, cyl_n, ang, vel, mu, rho):
    ...
    # Approximations per laminar and turbulent flow (3-8)
    ...
    cyl_cd = (cyl_cd * cyl_len * cyl_dia + np.pi * cyl_dia * cyl_cf) / (
...
def platedrag(plate_len, plate_thi, plate_wid, plate_n, ang, vel, mu, rho, q):
    ...
    pitch[abs(pitch) > 90] = abs(pitch[abs(pitch) > 90]) - 90
    signs = np.sign(pitch) + (pitch == 0) * 1  # get sign, watches for the zero case
    pitch = abs(pitch)
    ...
    # front face things
    plate_cf = plate_cf + (0.03 * (plate_thi / plate_thi) / (
    ...
    # For a plate with normal along the flow direction
    pitch = pitch * signs + 90  # realtive to x axis like alpha (AoA)
    pitch[abs(pitch) > 90] = (abs(pitch[abs(pitch) > 90]) - 90) * signs0[abs(pitch) > 90]
    signs = np.sign(pitch) + (pitch == 0) * 1  # get sign, watches for the zero case
    pitch = abs(pitch)
...
def ellipticaldrag(ell_chord, ell_len, ell_dia, ell_n, ang, vel, mu, rho):
    ...
    keep = ell_Re <= 10 ** 6  # if above transtion Reynolds number, then we make it turbulent
    ...
    # (7-19) Figure 33 shows the lift curve slop as a function of spar area drag, which can be used for find the lift for a round fuselage/steamline body.
    ell_cdb = ell_cd * (np.pi * (
                ell_dia / 2) ** 2) * ell_dia ** 2  # we have to do a conversion here as area = ell_dia^2 for this drag coefficient, not the typical area = pi*r^2
    # We will create a linear expression (0.012 / 0.15) = 0.08
    ell_cla = 0.08 * ell_cdb
    ell_cla[ell_cdb > 0.2] = 0.5 * np.pi  # the relationship plateaus after cd_b > 0.2
    ell_cl = ell_cla * pitch
    ...
    # Combine friction and pressure drag
    ell_cd = (ell_cd * np.pi * (ell_dia / 2) ** 2 + np.pi * ell_cf) / (np.pi * (ell_dia / 2) ** 2)  # (3-11)
    # Calculate reference area
...
def run_full(DataName, ParaName, include_wing, create_plot, debug, stl_output):
    vp = 5  # 30 m/s
    ap = 4  # +0 deg
    mp = 100  # 50#scale of arrows? 50 for UAM, 100 for UAV works well

    vel = np.arange(5, 51, 5)  # Range of velocties for table
    ...
    if stl_output == True:
        Mesh_list = list()
    prop={}    
    #####7/8/22Unfinished prop interference code
    ...
    #         # prop mesh with diamter * 2 for interference projection
    #         mesh = trimesh.creation.cylinder(float(parameter[q]["DIAMETER"]), float(parameter[q]["HUB_THICKNESS"]),
    ...
    for o in np.arange(1,4, 1):  # change second digit to 4  to get all 3 directions
        ...
        for q in spatial:
            ...
            # 7/8/22 we are ignoring batteries as they should be in a fuselage now!
            ...
            #### UAM parts
                ...
                spatial[q]['GCG'] = np.array(spatial[q]['CG'], dtype=float) + np.array(spatial[q]['Translation'],
                                                                                       dtype=float)
            ...
            if 'mesh' in locals() and 'cd' in locals():  # "prop" not in parameter[q]['CADPART']and "wing" not in parameter[q]['CADPART']:
                ...
                spatial[q]['Poly'] = poly
                spatial[q]['X_BB'] = np.sort(mesh.bounds[:, 0])[
                                     ::-1]  # this is the x coordinates for a bounding box, larger x is always towards front / in coming flow
                
                # ###### 7/8/22 Unfinished prop interference code
                ...
                #             # pinfl = pitem.intersection(prop[qqq]["Poly"]).area / pitem.area
                #             prop[qqq]["polyp"].append([q,pitem.intersection(prop[qqq]["Poly"]).area / pitem.area])
                #             if "wing" in parameter[q]["CADPART"]:
                #                 plt.figure()
                #                 trimesh.path.polygons.plot([pitem,prop[qqq]["Poly"]], label = [q, qqq])
                #                 plt.legend()
        ...
        interf = dict(sorted(spatial.items(), key=lambda item: item[1]["GCG"][0], reverse=True))
                    ...
                    if aft == 1:
                        ...
                        xd = distlength(
                            [interf[a]['X_BB'][0], interf[q]['X_BB'][1], interf[a]['GCG'][1], interf[q]['GCG'][1],
                             interf[a]['GCG'][2], interf[q]['GCG'][
                                 2]]) / L  # distance between CGs / characteristic length. Is newer rear face to front face method
                        ...
                        if mf < 1 and IA > 0:  # A little odd here because IA=1 says big area overlap, but mf=1 says no influence
                            # Proportion to modfun of front object and fraction of intersected area of front object
                            spatial[a]['marea'][0, qqq] = spatial[a]['marea'][
                                                              0, qqq] + 1  # Number of times area/region interacts with other objects in flow

                            spatial[a]['mfun'][int(spatial[a]['marea'][0, qqq] - 1), qqq] = 1 - (
                                        1 - mf) * IA  # Save modifying scalar, multiply by area of overlap ratio
        ...
        for q in spatial:
            if "Cd" in spatial[q]:
                ...
                spatial[q]["Drag"] = 0.5 * rho * Vel ** 2 * spatial[q]['Cd'] * np.tile(spatial[q]['rarea'],
                                                                                       [np.size(Vel, axis=0),
                                                                                        1]) * np.tile(modder,
                                                                                                      [len(vel), 1])
    ...
    if debug==True:
        return (drags, center, spatial, parameter, prop)
    else:
        return (drags, center)
...
if __name__ == "__main__":
    ...
    DB = False
    if DB==True:
        drags, center, spatial, parameter, prop = run_full(DataName='designData.json', ParaName='designParameters.json', include_wing=True, create_plot=True,
                 debug=True, stl_output=True)
    else:
        drags, center = run_full(DataName='designData.json', ParaName='designParameters.json', include_wing=True, create_plot=False,
                 debug=False, stl_output=True)
