...
c	The best description of the internal memory is looking at parts.h
c	and the comments at the top of subroutine fderiv below.
c	James Walker  16 March 2021  Southwest Research Institute
c	4/12/2021
c
...
c	adjust those units.  Thus, parts includes a modified for length
...
c	conversion factor (located in parts.h) to 1.
...
c	We need to think about input dimensions.
	...
	write (6,*) '  This is the output file for the SwRI flight'
     .       ,' dynamics model.'
c	It still contains this code, but now it is a given that it does.
c	write (6,*)                                                      !  8/25/21
c	write (6,*) '  It includes the Hackathon 1 8/25/21 capability'   !  8/25/21
c     .                ,' to specify a lateral trim state for hover.'     !  8/25/21
...
c	the same controller.
	...
	write (6,*) 
	write (6,*) ' The body  frame is frd: x forward, y right,'
     .              ,' z down (down is positive) '
	write (6,*) ' The world frame is ned: X north,   Y east, '
     .              ,' Z down (down is positive, so altitude (up)'
     .              ,' is negative)'

	write (6,*)
	write (6,101) xdim
 101	format(' Number of state variables x: xdim = ', i3) 
	write (6,*) '    x( 1 -  3) is velocity in body frame: '
     .                   ,'U forward, V right, W down (m/s)'
	write (6,*) '    x( 4 -  6) is angular velocity in body frame:'
     .                   ,' P about x (roll rate), '
     .                   ,'Q about y (pitch rate), '
     .                   ,'R about z (yaw rate) (radians/s)'
	write (6,*) '    x( 7 - 10) is quaternion connecting world'
     .                   ,' frame to body frame: q0, q1, q2, q3'
	write (6,*) '    x(11 - 13) is world frame displacement: '
     .                   ,' X north, Y east, Z down'
     .                   ,' (m)'
	if (aircraft%num_propellers.gt.0) then
	   write (6,103) 13+aircraft%num_propellers
 103       format('     x(14 - ',i2') is motor angular speed (one for'
     .           ,' each motor) (radians/s)')
	endif
c	if (aircraft%num_controls.gt.aircraft%num_propellers) then
c	   write (6,104) 13+aircraft%num_propellers+1,
c     .                   13+aircraft%num_controls
c 104       format('     x(',i2,' - ',i2,') is aileron, flap deflection,'
c     .             ,' etc.')
c	endif
	if (aircraft%num_batteries.gt.0) then
	   write (6,107) 13+aircraft%num_propellers+1,
     .              13+aircraft%num_propellers+aircraft%num_batteries
 107       format('     x(',i2,' - ',i2,') is battery charge (one for'
     .           ,' each battery) (amp seconds)')
	endif
	write (6,102) udim
 102	format(' Number of controls uc (0 <= uc <= 1): udim = ',i3)
	if (udim.gt.0) then
	   write (6,105) udim
 105       format('     uc( 1 - ',i2,') are control channels')
c105       format('     uc( 1 - ',i2,') is motor controller (one for'
c    .           ,' each motor)')
	endif
c	if (aircraft%num_controls.gt.aircraft%num_propellers) then
c	   write (6,106) aircraft%num_propellers+1,
c     .                   aircraft%num_controls
c 106      format('     uc(',i2,' - ',i2,') is aileron, flap controller,'
c     .             ,' etc.')
c	endif
	do i = 1, aircraft%num_propellers
	   write (6,123) i, propeller(i)%icontrol, propeller(i)%ibattery
	enddo
	do i = 1, aircraft%num_wings
           write (6,124) i, 1, wing(i)%icontrol1, wing(i)%bias1
	   write (6,124) i, 2, wing(i)%icontrol2, wing(i)%bias2
	enddo
 123	format(15x,'  Propeller motor ',i2,' controlled by uc channel '
     .     ,i2,' and powered by battery ',i2)
 124	format(15x,'  Wing ',i2,' servo ',i1,' controlled by uc channel'
     .     ,' ',i2,' with bias ',f6.3)
	write (6,*)
...
c	uc(1:udim) = aircraft%uc_initial(1:udim) ! initial controls
	uc(1:min(10,udim)) = aircraft%uc_initial(1:min(10,udim)) ! initial controls   jdw   2/22/22

	write (6,*) ' Downward force (N) = mg = '
     .              ,sngl(aircraft%grav*aircraft%mass)
...
c	We open another path file for diagnostics
	open(57,file='path3.out',status='unknown')
...
c	We are now going to determine max thrust for each motor/propeller
	write (6,*)
	write (6,*) '  Information on propeller/motor performance'
     .              ,' assumes the vehicle is at rest.'
     .             ,'  In this list, the power and current are the'
	write (6,*) '  power and current supplied by the battery.'
     .             ,'  To obtain the current and power in the motor,'
     .              ,' it is necessary to reduce the'
	write (6,*) '  values by multiplying by efficiency_ESC.'
     .             , '  The Efficiency is the motor efficiency -'
     .              ,' i.e., there is no knockdown for the ESC.'
	write (6,*) '  When the three different approaches yield'
     .              ,' similar values for the motor/propeller/'
     .              ,'battery, then they are appropriately sized.'
	write (6,*) '  Otherwise, there is mass inefficiency.'
	write (6,*)
	write (6,*) '  The Max Volt entry uses the battery voltage and'
     .             ,' the motor and propeller properties.  If the'
     .             ,' resulting power and current are less'
	write (6,*) '  than the'
     .             ,' motor rated values then a larger'
     .             ,' voltage battery or a larger propeller or'
     .             ,' a smaller motor is suggested.'
	write (6,*)
	write (6,*) '  The Max Power and Max Amps use the power and'
     .             ,' current rating from the motor, respectively,'
     .             ,' and the motor and propeller properties'
	write (6,*) '  (no battery properties are used).'
     .             ,'  An appropriate battery voltage'
     .             ,' for the motor can be estimated by motor max power'
     .             ,'/max current.'
	write (6,*)
	write (6,*) '                                '
     .            ,'                     '      
...
c	      Same as above, only this a is a/Cp above
     ...
     .          /((J2 - J1)*(Om2 - Om1))
	   ...
	   c   = - voltage + Rw*I0
	   omega = (-b+sqrt(b**2-4.d0*a*c))/(2.d0*a)
           n = omega/twopi   ! added 4/7/22
c	   Ct = (-(omega-Om2)*Ct11+(omega-Om1)*Ct21
c     .          /(Om2 - Om1)
...
c           write (6,*) '  Torque electrical '
c     .                ,(Kt/Rw)*(voltage - omega/Kv - Rw*I0)
...
c	   Power is electrical power
 ...
 10	   format(a10, i2, 12f13.2)   ! by request  3/23/2022
c 10	   format(a10, i2, 12f10.2)
...
c	James Walker  26 March 2021
c	3/26/21
c       8/17/22  Looking at variable speed flight paths.  JDW
	...
	double precision I_peak, Power_peak
	double precision :: ceiling = 0.001d0  ! some numerical space

        double precision a, ac, as, P, Q, R  ! for the new quaternion update  jdw  5/17/22

        double precision :: path_data(1000) = 0.d0    ! for path information, to avoid passing commons.  jdw  9/27/22

c       New variables for the filght path 9 scoring routine.
	double precision flightdist
        integer :: iflightscore = 0
        logical :: isin  = .true.  ! "is in"
        logical :: isout = .false. ! "is out"
        double precision :: error9    = 0.d0
        double precision :: error9max = 0.d0
        double precision :: time9out  = 0.d0
...
c	if (time.gt.2.795d0) aircraft%temp_debug = .true.
...
c	We are using array componentwise multiplies in F90
	x(1:13)    = aircraft%x_initial(1:13)    ! non wind speed/air density initial conditions
c	uc(1:udim) = aircraft%uc_initial(1:udim) ! initial controls
	uc(1:min(10,udim)) = aircraft%uc_initial(1:min(10,udim)) ! initial controls  2/22/22
...
c	a transient solution state), since we presume people
	...
	i_Hackathon   = control%i_Hackathon
	i_flight_path = control%i_flight_path
	timeslip      = control%timeslip
	speed         = control%requested_lateral_speed

	do i = 1, 2
	   if (i.eq.1) j = 6
	   if (i.eq.2) j = aircraft%i_out_metrics
	   write (j,*)
	   write (j,*) ' Hackathon ',control%i_Hackathon
 	   write (j,*) ' Path performance, flight path '
     .                ,control%i_flight_path
	enddo

c	Some special coding for the vertical ascent setup.
c	It sets the speed to the vertical speed and it picks
c	as the control law the smallest lateral speed
c	successfully computed control law.
	if (i_flight_path.eq.4) then
	   speed =  control%requested_vertical_speed
	   i = int(control%requested_lateral_speed + 0.1d0)  ! new 8/25/21, new capability to set lateral trim state
	   if (control%lateral_trim_if(i)) goto 112          ! new 8/25/21, new capability to set lateral trim state
	   do j = 0, 50
	      if (control%lateral_trim_if(j)) then
                 i = j
	         control%requested_lateral_speed = dfloat(i) ! new 8/25/21, needs to be set for autopilot; bug fix
	         goto 112
	      endif
	   enddo
	endif
...
c       This is setup for flight path 9   jdw  9/27/22

        if ((i_Hackathon.eq.1).and.(i_flight_path.ge.6)) then

c          Now we move all these terms for the requested accelerations
           path_data( 1) = control%requested_vertical_speed
           path_data( 2) = control%requested_vertical_down_speed
           path_data( 3) = control%requested_lateral_speed
           path_data( 4) = control%vertical_landing_approach_speed
           path_data( 5) = control%vertical_landing_speed_at_ground
           path_data( 6) = control%requested_vertical_acceleration
           path_data( 7) = control%requested_vertical_deceleration
           path_data( 8) = control%landing_approach_height
           path_data( 9) = control%requested_lateral_acceleration
           path_data(10) = control%requested_lateral_deceleration

           if (i_flight_path.eq.6)
     .        path_data(11) = dfloat(control%start_trim_state)

        endif

        if ((i_Hackathon.eq.1).and.(i_flight_path.eq.9)) then

           call initialize_9H1(path_data)

        endif


c	This is the coding to compute a trim state. We will then figure
c	out how it fits into the flow
c
c
c	do i = 0, 20
c 
...
c	This is a question of whether we want to start with a trim state
	i     = control%start_trim_state
        if (i.lt.0) then
	   j = int(speed+0.1d0)
	   if (control%lateral_trim_if(j)) i = j
	endif

c       Unfortunately, another special case   jdw  9/27/22
c       For this case, we start at rest and we always are set to 1 for vertical ascent.
        if ((i_Hackathon.eq.1).and.(i_flight_path.eq.9)) i = 1



c	if ((i_flight_path.eq.3).or.(i_flight_path.eq.5)) then
c	   iturn = control%start_trim_state
c           if (iturn.lt.0) then
c	      j = int(speed+0.1d0)
c	      if (control%turn_1_trim_if(j)) iturn = j
c	   endif
c	endif
...
c	Now let's explore the effect of roll on the trim state.
c	if (i.ge.0) then
c	   do j = 1, xdim
c              x(j) = control%lateral_trim_x(i,j)
c	   enddo
c	   do j = 1, udim
c	      uc(j) = control%lateral_trim_uc(i,j)
c	   enddo
c	   write (6,*) ' Initialized to trim state ',i
c	   write (6,*)
c	   write (6,*) ' x = ',x
c	   write (6,*) ' uc = ',uc
c
...
c	   do l = 0, 15
c
c	      phi = dfloat(l)*(pi/180.)/15.d0
c
c	      Reset the trim state
c	      do j = 1, xdim
c                 x(j) = control%lateral_trim_x(i,j)
c	      enddo
c
c	      To do the roll after the pitch (which the trim state already has)
c	      we muliply on the left.
c	      We need to move the PQR world vector into the appropriate frame
c
c	      q0phi = cos(phi/2.d0)
c	      q1phi = sin(phi/2.d0)
c	      q2phi = 0.d0
c	      q3phi = 0.d0
c
c	      q0 = x( 7)*q0phi - x( 8)*q1phi - x( 9)*q2phi - x(10)*q3phi
c	      q1 = x( 8)*q0phi + x( 7)*q1phi - x(10)*q2phi + x( 9)*q3phi
c	      q2 = x( 9)*q0phi + x(10)*q1phi + x( 7)*q2phi - x( 8)*q3phi
c	      q3 = x(10)*q0phi - x( 9)*q1phi + x( 8)*q2phi + x( 7)*q3phi
cc
...
c
c	      The rotation
c	      x(4) = xo(11)*UVWPQR(4) + xo(12)*UVWPQR(5) + xo(13)*UVWPQR(6)
c	      x(5) = xo(14)*UVWPQR(4) + xo(15)*UVWPQR(5) + xo(16)*UVWPQR(6)
c	      x(6) = xo(17)*UVWPQR(4) + xo(18)*UVWPQR(5) + xo(19)*UVWPQR(6)
c
c	      x( 7) = q0
c	      x( 8) = q1
c	      x( 9) = q2
c	      x(10) = q3
c
c	      call fderiv(time, xdim, x, xd, xo, udim, uc,
c     .                 aircraft, wing, propeller, battery, control)
c
cc	      write out the acceleration terms
c	      write (6,*) ' l, xd(1:6) ',l,sngl(xd(1:6))
c
c	   enddo
c	endif
	...
	if (i.ge.0) then
	   do j = 1, xdim
              x(j) = control%lateral_trim_x(i,j)
	   enddo
	   do j = 1, udim
	      uc(j) = control%lateral_trim_uc(i,j)
	   enddo
	   do j = 1, udim
	      do l = 1, 12
	         control%K(j,l) = control%lateral_trim_K(i,j,l)
	      enddo
	   enddo

c	   Right now we assume the correct turning motion 
c	   for path 3 is in turn_1 trim state.

	   if ((i_flight_path.eq.3).and.(i.ge.1)
     .         .and.(control%turn_1_trim_if(i))) then
	      do j = 1, xdim
                 x(j) = control%turn_1_trim_x(i,j)
	      enddo
	      do j = 1, udim
	         uc(j) = control%turn_1_trim_uc(i,j)
	      enddo
c	      The default control, in case the autopilot doesn't find something better
	      do j = 1, udim
		 do l = 1, 12
		    control%K(j,l) = control%turn_1_trim_K(i,j,l)
		 enddo
	      enddo
	      write (6,*) ' Initialized to turn_1 trim state ',i
	   else
	      write (6,*) ' Initialized to lateral trim state ',i
	   endif
	   write (6,*)
c	   write (6,*) ' x = ',x
c	   write (6,56) ' 0 uc = ',uc(1:udim)
...
c	   We rotate to the initial orientation.
c	   The dot product of (1,0) and (Xpathdot,Ypathdot) for the yaw psi.
	   temp   = sqrt(Xpathdot(1)**2+Xpathdot(2)**2)
	   if (temp.gt.0.d0) then
	      cospsi = Xpathdot(1)/temp  ! not consider Z direction
	      sinpsi = Xpathdot(2)/temp
	   else
...
c	   Now let's be very clear.  Psi for us is the angle that takes
c	   the world frame to the body frame.  Notice that the trim state
c	   is in the world frame along the X axis.  We want to move
c	   our trim state to the world state.  In that motion, the
c	   UVW and PQR are the same (because they are the body frame)
c	   but the quaternion is now different.  We want to rotate the
c	   trim state body frame to the world frame that matches Xpath.
c	   Since the quaternion connects the world frame to the body
c	   frame, it should have the same sign as psi computed above.
c	   rotation
...
c	write (6,*) ' Initial cospsi, sinpsi, cospsi/2,'
c     .        ,' sinpsi/2, angle, angle/2 '
...
c	   A marvelous thing about quaternions is that we we do subsquent rotations
c	   the multiplication is left to right - i.e. q1 * q2 is the same as R(q2)R(q1).
c	   We want to do our rotation about the Z axis first, before the trim state
c	   rotation about the y axis.  The quaternion that gives the rotation about
c	   the z axis is q_psi = (cos(psi/2)  0  0  sin(psi/2)).
c	   q_psi * q_trim is 
	
	   q0 = coshalfpsi*x( 7) - sinhalfpsi*x(10)
	   q1 = coshalfpsi*x( 8) - sinhalfpsi*x( 9)
	   q2 = coshalfpsi*x( 9) + sinhalfpsi*x( 8)
	   q3 = coshalfpsi*x(10) + sinhalfpsi*x( 7)
...
c	   Set the initial location.  This is stright from the flight
c	   path, so it does not need to be adjusted.
	   x(11) = Xpath(1)
	   x(12) = Xpath(2)
	   x(13) = Xpath(3)
...
c	   write (6,*) ' cospsi, sinpsi, coshalfpsi, sinhalfpsi '
c     .                  ,cospsi, sinpsi, coshalfpsi, sinhalfpsi

c          For path 9, we force the intial speed to be zero,
c          even though we are using the lateral 1 m/s trim state.  jdw 10/19/22
c           if ((i_Hackathon.eq.1).and.(i_flight_path.eq.9)) then
c              x(1) = 0.d0
c              x(2) = 0.d0
c              x(3) = 0.d0
c           endif
c          Maybe someday we will do this - it appears to be slightly
c          more stable on liftoff to stick with the 1 m/s lateral
c          initialization.  It drifts off course about the same with our without
c          the zero initialization.
...
c	aircraft%debug = 1
	...
	write (6,*) '    time       phi      theta      psi '
     .           ,'    Unorth     Veast     Wdown  '
     .             ,'  Xnorth     Yeast     Zdown'
     .           ,'      Vt       alpha     beta'
     .          ,'     Thrust     Lift      Drag'
     .          ,' itrim'
	write (6,*) '     (s)      (deg)     (deg)     (deg) '
     .            ,'    (m/s)     (m/s)     (m/s)  '
     .             ,'    (m)       (m)       (m) '
     .           ,'     (m/s)     (deg)     (deg)'
     .           ,'      (N)       (N)       (N)'
...
c                      123456789012123456789012123456789012
     .               ,'    Ypath          Z        Zpath   '
     .               ,'    Xdiff       Ydiff       Zdiff  '
     .               ,'Longitude err  L1_error  L1_error_max'
c     .               ,' L1_error_int'
     .               ,' Xpathdot'
...
c                      123456789012123456789012123456789012
	...
	time_out = time - dt_output/10000.d0  ! to force an initial write
	...
	do while ((time.lt.time_end).and.(.not.icomplete)
     .         .and.(.not.isout))
           ...
           if (time.gt.15.d0.and.time.lt.20.d0) then
              aircraft%debug = 1
           else
              aircraft%debug = 0
           endif  ! jdw  6/1/22
c           if (time.gt.15.d0.and.time.lt.20.d0)
c     .           write (6,96) 0,time,P,Q,R,sqrt(P*P+Q*Q+R*R),uc(1:udim)
c     .                         ,xo(5:10)
c           if (time.gt.15.d0.and.time.lt.20.d0)
c     .           write (96,96) 0,time,P,Q,R,sqrt(P*P+Q*Q+R*R),uc(1:udim)
c     .                         ,xo(5:10)
	   call flightpath(i_Hackathon, i_flight_path, time, timeslip
     .          , speed, Xpath, Xpathdot, lstraight, radius, icomplete
     .          , path_data)
	   if (control%external_autopilot.eq.1) then
	     call autopilotRedirect(time, xdim, x, xd, xo, udim, uc,
     .                  aircraft, wing, propeller, battery, control,
     .                  Xpath, Xpathdot, lstraight, radius,
     .                  i_flight_path)
	   else
     ...
     .                aircraft, wing, propeller, battery, control)
c           if (time.gt.15.d0.and.time.lt.20.d0)
c     .           write (6,96) 1,time,P,Q,R,sqrt(P*P+Q*Q+R*R),uc(1:udim)
c           if (time.gt.15.d0.and.time.lt.20.d0)
c     .           write (96,96) 1,time,P,Q,R,sqrt(P*P+Q*Q+R*R),uc(1:udim)
...
c          Some fairly complicated coding to end up with an exact solution
c          (no drift) for the quaternions, since that seems to still
c          be a problem with some vehicles when they don't fly well.
c          We are overwriting both q and dq/dt in the Runge-Kutta
c          solution with our exact integration.  jdw  5/17/22

c          We store information for the exponential quaternion update.
c          jdw  5/17/22
           P  = x( 4)
           Q  = x( 5)
           R  = x( 6)
...
c          We now adjust to have no drift in the quaternian, using an
c          exact exponential solution.  jdw  5/17/22
c          We are overwritting the Runge-Kutta solution.
           a  = sqrt(P**2 + Q**2 + R**2)
           if (a.eq.0.d0) then
              ac = 1.d0
              as = 0.d0
           else
              ac = cos(a*(0.5d0*dt)/2.d0)    ! a half time step for k2(x1) 
              as = sin(a*(0.5d0*dt)/2.d0)/a
           endif
           x1( 7) = ac*q0 + as*(      - P*q1 - Q*q2 - R*q3)
           x1( 8) = ac*q1 + as*( P*q0        + R*q2 - Q*q3)
           x1( 9) = ac*q2 + as*( Q*q0 - R*q1        + P*q3)
           x1(10) = ac*q3 + as*( R*q0 + Q*q1 - P*q2       )

	   call flightpath(i_Hackathon, i_flight_path, time1, timeslip
     .          , speed, Xpath, Xpathdot, lstraight, radius, icomplete
     .          , path_data)
	   if (control%external_autopilot.eq.1) then
	     call autopilotRedirect(time1, xdim, x1, xd, xo, udim, uc,
     .                  aircraft, wing, propeller, battery, control,
     .                  Xpath, Xpathdot, lstraight, radius,
     .                  i_flight_path)
	   else
     ...
     .                 aircraft, wing, propeller, battery, control)
c           if (time.gt.15.d0.and.time.lt.20.d0)
c     .           write (6,96) 2,time,P,Q,R,sqrt(P*P+Q*Q+R*R),uc(1:udim)
c           if (time.gt.15.d0.and.time.lt.20.d0)
c     .           write (96,96) 2,time,P,Q,R,sqrt(P*P+Q*Q+R*R),uc(1:udim)
...
c          We are overwritting the Runge-Kutta solution.
           ...
           if (a.eq.0.d0) then
              ...
              ac = cos(a*(0.5d0*dt)/2.d0)    ! a half time step for k3(x2)
              as = sin(a*(0.5d0*dt)/2.d0)/a
           endif
           x2( 7) = ac*q0 + as*(      - P*q1 - Q*q2 - R*q3)
           x2( 8) = ac*q1 + as*( P*q0        + R*q2 - Q*q3)
           x2( 9) = ac*q2 + as*( Q*q0 - R*q1        + P*q3)
           x2(10) = ac*q3 + as*( R*q0 + Q*q1 - P*q2       )

	   call flightpath(i_Hackathon, i_flight_path, time2, timeslip
     .          , speed, Xpath, Xpathdot, lstraight, radius, icomplete
     .          , path_data)
	   if (control%external_autopilot.eq.1) then
	     call autopilotRedirect(time2, xdim, x2, xd, xo, udim, uc,
     .                  aircraft, wing, propeller, battery, control,
     .                  Xpath, Xpathdot, lstraight, radius,
     .                  i_flight_path)
	   else
     ...
     .                 aircraft, wing, propeller, battery, control)
c           if (time.gt.15.d0.and.time.lt.20.d0)
c     .           write (6,96) 3,time,P,Q,R,sqrt(P*P+Q*Q+R*R),uc(1:udim)
c           if (time.gt.15.d0.and.time.lt.20.d0)
c     .           write (96,96) 3,time,P,Q,R,sqrt(P*P+Q*Q+R*R),uc(1:udim)
...
c	write (6,56) ' DCM = ',xo(17),xo(18),xo(19)
	   ...
	   time3 = time + dt
           ...
           if (a.eq.0.d0) then
              ...
              ac = cos(a*dt/2.d0)    ! a full time step for k4(x3)
              as = sin(a*dt/2.d0)/a
           endif
           x3( 7) = ac*q0 + as*(      - P*q1 - Q*q2 - R*q3)
           x3( 8) = ac*q1 + as*( P*q0        + R*q2 - Q*q3)
           x3( 9) = ac*q2 + as*( Q*q0 - R*q1        + P*q3)
           x3(10) = ac*q3 + as*( R*q0 + Q*q1 - P*q2       )

	   call flightpath(i_Hackathon, i_flight_path, time3, timeslip
     .          , speed, Xpath, Xpathdot, lstraight, radius, icomplete
     .          , path_data)
	   if (control%external_autopilot.eq.1) then
	     call autopilotRedirect(time3, xdim, x3, xd, xo, udim, uc,
     .                  aircraft, wing, propeller, battery, control,
     .                  Xpath, Xpathdot, lstraight, radius,
     .                  i_flight_path)
	   else
     ...
     .                 aircraft, wing, propeller, battery, control)
c           if (time.gt.15.d0.and.time.lt.20.d0)
c     .           write (6,96) 4,time,P,Q,R,sqrt(P*P+Q*Q+R*R),uc(1:udim)
c           if (time.gt.15.d0.and.time.lt.20.d0)
c     .           write (96,96) 4,time,P,Q,R,sqrt(P*P+Q*Q+R*R),uc(1:udim)
...
c          Time to do our final/actual update of the quaternion.
c          What do we have?  We now have a P at the beginning
c          of the time step and a P at the end.  We choose to use an
c          average value of P for our P value.  Then we do the
c          exact step with this P to update the quaternion.  5/17/22
           ...
           if (a.eq.0.d0) then
              ...
              ac = cos(a*dt/2.d0)    ! full time step
              as = sin(a*dt/2.d0)/a
           endif
           x( 7) = ac*q0 + as*(      - P*q1 - Q*q2 - R*q3)
           x( 8) = ac*q1 + as*( P*q0        + R*q2 - Q*q3)
           x( 9) = ac*q2 + as*( Q*q0 - R*q1        + P*q3)
           x(10) = ac*q3 + as*( R*q0 + Q*q1 - P*q2       )
c           if (time.gt.15.d0.and.time.lt.20.d0)
c     .           write (6,96) 5,time,P,Q,R,sqrt(P*P+Q*Q+R*R),uc(1:udim)
c           if (time.gt.15.d0.and.time.lt.20.d0)
c     .           write (96,96) 5,time,P,Q,R,sqrt(P*P+Q*Q+R*R),uc(1:udim)
 96        format('  Spin ',i1,20f12.4)


c          We are having issues with drift still (jdw 4/29/22).
c          One approach is to normalize the quaternion after the
c          update.  Let's see if that resolves the problem.
c           temp  = sqrt(x(7)**2 + x(8)**2 + x(9)**2 + x(10)**2)
c           x(7)  = x(7)/temp
c           x(8)  = x(8)/temp
c           x(9)  = x(9)/temp
c           x(10) = x(10)/temp
c          jdw debug 5/5/22
c           write (6,*) ' time,q ',sngl(time),sngl(x(7)),sngl(x(8))
c     .                 ,sngl(x(9)),sngl(x(10))
c     .                 ,sngl(x(7)**2+x(8)**2+x(9)**2+x(10)**2)
c     .                 ,sngl(sqrt(x(1)**2+x(2)**2+x(3)**2))
...
c          For flight path 9 we have another path error estimate.
           if ((i_Hackathon.eq.1).and.(i_flight_path.eq.9)) then
              call score9(X(11), X(12), X(13), flightdist, iflightscore
     .                  , isin, error9, path_data)
              error9max = max(error9, error9max)
              if (.not.isin) then
                 isout = .true.
                 time9out = time
              endif
c             We store the speed for ground impact speed
              ground_impact_speed = sqrt(xd(11)**2+xd(12)**2+xd(13)**2)
c
c             Previously we had the icomplete flag set in the autopilot.
c             However, we move this here so that any autopilot can be
c             scored correctly and also to resolve a potential false score
c             for vehicles that don't move.     jdw  10/19/22
c             Since we are not following the path here, per se, this
c             coding has the potential flaw of the previous coding
c             in that a vehicle can "jump" to the finish line, rather
c             than fly.  We will assume that possibility is hand-checked.
c             Here we just ask if it has crossed the finish line.
c
c             Notice this has hard wired components from the path and scoring
c             set up, namely 15 meters allowable error and the ending location.
c             Positive z means we should have penetrated the ground plane.
              icomplete = .false.
              if (isin) then
                 if ((((X(11) - 2400.d0)**2 + (X(12) - 1300.d0)**2)
     .                 .le. 15.d0**2)
     .             .and.
     .                (X(13).gt.0.d0))
     .           icomplete = .true.
...
c          This did not seem to work.  We may only want to try something
c          like this when there has been a breakdown and we are doing a
c          "one time" large reset.
...
c	      We are interested in time to 20% power
     ...
     .                    * battery(j)%Capacity/1000.d0)
	      ...
	      if (.not.aircraft%ignore_electrical) then
	         if (bat_capacity(j).gt.0.8d0+ceiling) goto 200
                 if (bat_amps    (j).gt.1.d0+ceiling ) goto 200
	      endif
	   enddo
	   do j = 1, aircraft%num_propellers
c	      Now we check for the current in the motors
c             We anticipate using a transient peak
              I_peak = propeller(j)%I_peak
              if (I_peak.eq.0.d0) 
     .               I_peak = max(aircraft%I_peak_ratio,1.d0)
     .                      * propeller(j)%I_max
     ...
     .               / I_peak
	      motor_amps(j) = max(ratio,motor_amps(j))
              I_peak = propeller(j)%I_peak
              if (I_peak.gt.0.d0) then
	         Power_peak = max(I_peak/propeller(j)%I_max,1.d0)
     .                      * propeller(j)%maxpower              ! we boost this also for transients
              else 
	         Power_peak = max(aircraft%I_peak_ratio,1.d0)
     .                      * propeller(j)%maxpower              ! we boost this also for transients
	      endif
	      ratio = xo(29 + aircraft%num_propellers + j)
	      ratio = xo(29 + j)
     ...
     .               / Power_peak
	      ...
	      if (.not.aircraft%ignore_electrical) then
	         if (motor_amps (j).gt.1.d0+ceiling) goto 200
	         if (motor_power(j).gt.1.d0+ceiling) goto 200
	      endif
	   enddo


c       jdwdebug 4/25/22
c        if (time.gt.57.d0) aircraft%debug = 1
c        if (time.gt.58.d0) aircraft%debug = 0
     ...
     .         ,control%i_trim_state
 10	      format (16f10.3,i4)

c       jdwdebug 4/25/22
c        write (6,*) ' Drag = ',sqrt(xo(23)**2+xo(24)**2+xo(25)**2),
c     .                         xo(23),xo(24),xo(25)
c        write (6,*) ' Spin ',sngl(P),sngl(Q),sngl(R)
c        if (time.gt.106.d0) time_out = time_out - dt_output
...
c     .            , l1_error_int
     .            , sqrt(Xpathdot(1)**2+Xpathdot(2)**2+Xpathdot(3)**2)
 55	      format(14e12.4)
c	     Phi, theta, psi
	...
	enddo  ! on time, completion, or path 9 exiting
	...
	lelectrical = .false.
     ...
     .                     ' due to set time_end.'
           ...
           write (6,*) ' Exited allowable volume at time ',sngl(time)
 ...
 220       format('   Battery ',i2,'  fraction capacity used '
     ...
     .                ,' max continuous amperage used ',f10.4)
	   if (bat_capacity(j).gt.0.8d0+ceiling) lfail = .true.
	   if (bat_amps(j)    .gt.1.0d0+ceiling) lfail = .true.
	enddo
	do j = 1, aircraft%num_propellers
 ...
 221       format('   Motor ',i2,' fraction max amps reached '
     ...
     .                 ,'power reached ',f10.4)
	   if (motor_amps(j) .gt.1.d0+ceiling) lfail = .true.
	   if (motor_power(j).gt.1.d0+ceiling) lfail = .true.
	enddo
...
c	      score = path_distance/10.d0
c	      Write (6,*) ' Score is distance flown / 10 = ',sngl(score)
c     .             ,' since minimum flight distance of 2 km achieved'
c	      score = min(score, 400.d0)
c	      Write (6,*) ' The maximum score on flight distance is 400'
c     .              ,' so score is = ',sngl(score)
c             The modified score for the Feb/Mar/April 2022 challenge
	      if (path_distance.le.4000.d0) then
                 score = path_distance/10.d0
              else
                 score = 400.d0 + (path_distance - 4000.d0)/100.d0
              endif
	      Write (6,*) ' Score is distance flown / 10 (up to 4 km)'
     .             ,' and /100 beyond that = ',sngl(score)
...
c	   The racing oval
	...
	else if (i_flight_path.eq.6) then
c	   Straight line flight
           write (6,*) ' Acceleration test path where input initial'
     .                ,' speed, final speed, and requested'
     .                ,' acceleration or deceleration determine'
     .                ,' distance and time.'
           write (6,*) '    Requested initial speed (from '
     .                ,'start_trim_state): ',sngl(path_data(11))
           write (6,*) '    Requested final speed (from '
     .                ,'requested_lateral_speed): ',sngl(path_data(3))
     .                ,'  Actual final speed = '
     .                ,sngl( sqrt(xd(11)**2+xd(12)**2+xd(13)**2))
           write (6,*) '    Requested acceleration (+) or deceleration'
     .                ,' (-): ',sngl(path_data(14))
           write (6,*) '    Expected distance: ',sngl(path_data(12))
     .                     ,'  Distance flown = ',sngl(path_distance)
           write (6,*) '    Expected time: ',    sngl(path_data(13))
     .                 ,'  Actual time = ', sngl(time)
	   score = path_distance/10.d0 - 10.d0*l_max_error
	   score = max(score, 0.d0)
           write (6,*) '    This is not a scored path, but a score of'
     .                ,' path distance/10  - 10 * error >= 0'
     .                ,' is provided for analysis: ', score

	else if (i_flight_path.eq.9) then

           score = float(iflightscore)
           write (6,*) ' Maximum distance error'
     .                ,' (measured perpendicularly) by Path 9 scoring '
     .                ,' routine ', sngl(error9max),' m'
           write (6,*)
           write (6,*) ' Path 9 distance traveled within allowed'
     .                ,' bounds: ',sngl(flightdist),'m'
           if (isout) write (6,*) ' Exited allowable volume at time '
     .                 ,sngl(time)
           write (6,*) ' Base score based on distance traveled and '
     .                 ,'potential bonuses for transitions '
     .                 ,iflightscore
           write (6,*) '   (Distance/10 + 200 + 200) '
           if (icomplete) then
              write (6,*) ' Path 9 completed, from take off '
     .                    ,'to landing.'
              write (6,*) '   Downward speed at landing was '
     .                     ,sngl(ground_impact_speed), ' m/s'
              if (ground_impact_speed.gt.0.2d0) then
                 write (6,*)'   No bonus points owing to crash landing '
     .                       , '(greater than 0.2 m/s). '
              else
                 write (6,*) '   Bonus +1000 for completion and'
     .                     , ' nice landing.'
                 write (6,*) '   Bonus ',max(0.,sngl(1000.d0-time)),
     .                       ' for time less than 1000 seconds.'
                 score = score + 1000.d0 + max(0.d0,1000.d0-time)
              endif
           else
              write (6,*) ' Path 9 not completed.'
              ground_impact_speed = 0.d0
           endif
	endif
	...
	if (aircraft%ignore_wingload) then
	   write (6,*) ' Score ignoring wing loading'
     .                  ,sngl(path_score)
           path_score = 0.d0
	   write (6,*) ' However, score set to 0 because of wing loading'
...
c	The scoring write-out for the path 
	do i = 1, 2
     ...
     .      ,'meters per second)'
	   write (j,*)
	   if ((path_score.gt.0.d0).and.(.not.isout)
     .          .and.(.not.lelectrical)) then
     ...
     .          ,sngl(control%R)
           write (j,*) 'Battery_fraction_used ',
     .          (sngl(bat_capacity(l)),l = 1, aircraft%num_batteries)
...
cc	described in subroutine fderiv and parts.h
...
c	integer xdim,udim
c
c	double precision time, x(xdim), xd(xdim), xo(1500), uc(udim)
c
...
c	Power for each motor based on battery voltage (i) = xo(29+3*numpropellers+i) i.e., not motor voltage 
c	Total power based on battery voltage = xo(29+4*numpropellers+1) = Power_tot_BV (just one value)
c
c       Total thrust per propeller  xo(30+4*numpropellers+i)
c       Total lift per wing         x0(30+5*numpropellers+i)
c
c
...
c	the appropriate calls are all put in place.  6/9/21
	...
	double precision a, b, c, Kt, Kv, Rw, omega, amps, Power_tot, qq
	integer          jdw
	double precision omega0, omega1, ff, ffp, a1, b1, omega2, DeltaJ
	...
	double precision Jmax, omegamax, omegamax2, Vmax
	...
	double precision Power_tot_BV, I_peak, voltage1

	logical idebugtag

	idebugtag = .false.

	if (aircraft%debug.ne.0) then  ! changed for interior setting jdw 2/2/22
           ip = .true.	
	else
	   ip = .false.
	endif

c	if (ip) write (6,*) ' Fderiv Time = ',time
	kpast = 0
	lpast = 0
...
c	if (ip) write (6,*) ' uwwind ',Uwind,Vwind,Wwind
...
c	if (ip) write (6,*) ' upwind ',Up,Vp,Wp
...
c       Updated body drag routine to increase moments (parallel axis theorem)
c       AJC 6/2/22
        call body_drag(rho,Viw,Vp,Wp,P,Q,R
     ...
     .      ,aircraft%mass, aircraft%Ixx, aircraft%Iyy, aircraft%Izz
     .      ,Fx,Fy,Fz,Mx,My,Mz)

c       We know these are the first ones, so we reset them.
	Fx_tot = Fx
	Fy_tot = Fy
	Fz_tot = Fz
...
c       Let's do something fake, trying to hunt down the issue  jdw 6/1/22
c	Mx_tot = Mx_tot - abs(P)*P
c	My_tot = My_tot - abs(Q)*Q
c	Mz_tot = Mz_tot - abs(R)*R

c        if (time.gt.15.d0.and.time.lt.20.d0)
c     .     write (96,100) 'body drag',0,1,
c     .                Fx_tot, Fy_tot, Fz_tot
c     .               ,Mx_tot, My_tot, Mz_tot

    

c	if (ip) write (6,100) 'body drag',0,1, 
c     .                Fx_tot, Fy_tot, Fz_tot
c     .               ,Mx_tot, My_tot, Mz_tot
...
c	   I choose to take the total force on the wing, since we can
c          be at any angle.  It should be close to lift, though
c          Total lift per wing         x0(30+5*numpropellers+i)
	   xo(30+5*aircraft%num_propellers+i)
     .        = sqrt(Force_Lift_w**2 + Force_Drag_w**2)
...
c	   if (ip) write (6,100) 'wing lift and drag',i ,2,
c     .                Fx, Fy, Fz, Mx, My, Mz
...
c	      This is new to address the potential of flow faster than
c	      the spin provided by the voltage.  We assume that it
c	      spins freely - no power generation or braking).
c	      This next if is not well behaved - we get different
c	      behavior for le (better) vs. lt
c	      It seems to be better behaved when we separte these
c	      ifs - originally they were together, but separate they
c	      should be continuous, though not differentiable.  jdw  2/3/22
              if (Thrust.le.0.d0) then
	         if (ip) write (6,*) 
     .             ' Zeroing out Thrust: Thrust, Power, Torque ',
     .               Thrust, Power, Torque
	         Thrust = 0.d0
	      endif
              if (Power.le.0.d0) then
                 if (ip) write (6,*)
     .             ' Zeroing out Power and Torque:',
     .             ' Thrust, Power, Torque ',
     .               Thrust, Power, Torque
	         Power  = 0.d0
                 Torque = 0.d0
...
c	   We now compute the work the motor is doing.
     ...
     .             + propeller(i)%bc ! voltage signal from the controls.
	   ...
	   if (voltage.gt.0.d0) then
	      ...
	      if (aircraft%current_limiter) then
c	         This is the current based on the current motor spin
c	         We assume this was employed the previous cycle
c                limiting the voltage, so we just invoke it here.
	         I_peak = propeller(i)%I_peak
                 if (I_peak.eq.0.d0) 
     .              I_peak = max(1.d0,aircraft%I_peak_ratio) 
     .                     * propeller(i)%I_max
	         if (amps.gt.I_peak) then
c                   We limit the voltage
                    voltage1 = Rw * I_peak + motor_omega(i)/Kv
c                    if (voltage1.gt.voltage) write (6,*) 
c                    write (6,*) 
c     .                 ' Amp limited voltage: actual (limited),'
c     .                ,' requested ',i,voltage1,voltage
	            voltage = voltage1
                    amps = I_peak  ! this is the value that is stored
	         endif
	      endif

c	      We cannot go below this power usage.
	      I0   = propeller(i)%I_idle
	      if (amps.lt.I0) then
                 amps = I0
c	         if (ip) write (6,*) ' amps below I0 ',amps,I0  ! may need to check how this affects linearization  jdw  2/3/22
	      endif
...
c	      To address some behavior/trim convergence issues, I am also going to look at
c             the power based on the battery voltage and battery current.  This is a
c             slightly different thing, but I am having some odd behavior where the
c             trim states have higher current but less motor power.  jdw  2/24/22
	      xo(29 + 3*aircraft%num_propellers + i) 
     .           = battery(propeller(i)%ibattery)%voltage   ! batter voltage rather than motor voltage
     .            *amps/propeller(i)%efficiency_ESC         ! this amperage for each individual motor
	      Power_tot_BV = Power_tot_BV 
     .                       + xo(29 + 3*aircraft%num_propellers + i) 
	   endif
...
c	if (ip) then
c	   write (6,*) ' i,Torque, spin, uc, ',i,Torque
c     .                         ,propeller(i)%spin,uc(i)
c	endif
c
c	   if (ip) write (6,100) 'torque from prop rotation',
c     .        i, 3, 0.d0, 0.d0, 0.d0, Mx, My, Mz
...
c          Total thrust per propeller  xo(30+4*numpropellers+i)
           xo(30+4*aircraft%num_propellers+i) = Thrust
...
c	   if (ip) write (6,100) 'prop thrust/torque',i,4, 
c     .                Fx, Fy, Fz, Mx, My, Mz
...
c	   if (ip) write (6,100) 'prop gyroscopic',i,6,
c     .                0.d0, 0.d0, 0.d0
c     .               ,Mx, My, Mz
...
c	   This is our current limiter, which looks at where we are spin wise for the voltage.
	   if (aircraft%current_limiter) then
c	      This is the current based on the current motor spin
	      amps = (voltage - motor_omega(i)/Kv)/Rw
	      I_peak = propeller(i)%I_peak
...
c                We limit the voltage
                 voltage1 = Rw * I_peak + motor_omega(i)/Kv
                 if (voltage1.gt.voltage) write (6,*) 
     .             ' Voltage 1, voltage ', voltage1,voltage
	         voltage = voltage1
	      endif
	   endif
...
c	      Same as above, only this a is a/Cp above
	      ...
	      if (.false.) then ! <- notice this is all being skipped right now

c	      We now include coding to prevent a negative Cp (1/17/22)
	      n = omega/twopi  ! to get reveolutions per second from radians per second
	      J = Vpath / (n*D) ! J = V/nD
	      omegamax2 = Kv*(voltage - Rw*I0)
              Cp=(Cp11*(omega-O2)*(J-J2)-Cp21*(omega-O1)*(J-J2)
     .           +Cp22*(omega-O1)*(J-J1)-Cp12*(omega-O2)*(J-J1))
	      if (Cp.lt.0.d0) then
	         a = bb
		 b = aa*twopi*Vpath/D+dd
	         c = cc*twopi*Vpath/D
	         if (4.d0*a*c.le.b**2) then
	            if (b.ge.0.d0) then
                       qq = - (b + sqrt(b**2 - 4.d0*a*c))/2.d0
	            else   
                       qq = - (b - sqrt(b**2 - 4.d0*a*c))/2.d0
	            endif
	            write (6,*) ' new omega ',qq/a,c/qq
	         else
	            write (6,*) ' the determinent is less than zero '
	         endif
	      endif
	         Jmax = propeller(i)%J(propeller(i)%prop_num_pts_J)
	         omegamax = propeller(i)%omega
     .                           (propeller(i)%prop_num_pts_omega)
		 Vmax = D * (omegamax/twopi) * Jmax
	         write (6,*) ' Jmax, omegamax, omegamax2, Vmax, Vpath '
     .                 ,sngl(Jmax),sngl(omegamax),sngl(omegamax2),
     .                  sngl(Vmax),sngl(Vpath)
	        
     	      endif 

	      if (.false.) write (6,*) ' j,Omega, c, -ff/ffp '
     .                     ,jdw,omega,c,-ff/ffp
	      if (.false.) then
	         write (6,*) ' Kt(I-I0) = ',Kt*(voltage-omega/Kv-Rw*I0)/Rw
	         write (6,1122) a3,a2,a1,a0
 1122	         format(' a3,a2,a1,a0 ',4e15.7)
	         write (6,*) ' Cp11, Cp12, Cp21, Cp22 '
     .                 ,sngl(Cp11),sngl(Cp12),sngl(Cp21),sngl(Cp22)
	         n = omega/twopi  ! to get reveolutions per second from radians per second
	         J = Vpath / (n*D) ! J = V/nD
                 temp=(Cp11*(omega-O2)*(J-J2)-Cp21*(omega-O1)*(J-J2)
     .                +Cp22*(omega-O1)*(J-J1)-Cp12*(omega-O2)*(J-J1))
     .                /((O2-O1)*(J2-J1))
	         write (6,*) ' Omega, O1, O2, J, J1, J2 '
     .                       , Omega, O1, O2, J, J1, J2 
	         write (6,*)' k,l,ierr12,Cp, Power, Torque_mechanical ',
     .              k,l,ierror1,ierror2,temp, temp*rho*n**3*D**5,
     .                    temp*rho*n**3*D**5/motor_omega(i)
	         write (6,*) ' number of J points ',
     .                 propeller(i)%prop_num_pts_J
c	      Power  = Cp*rho*n**3*D**5      ! P = Cp*rho*n^3*D^5
c              Torque = Power/motor_omega(i)  ! Q = P/motor omega
...
c	   our first order ODE to head towards the the requested omega is
     ...
     .                    /propeller(i)%tau_ESC
	   if (.false.) write (6,*) ' omegadot ',i,motor_omegadot(i)
	   if (idebugtag) write (6,*) ' Based on omega ',i,
     .            sngl((voltage - omega/Kv)/Rw)
     .           ,sngl(propeller(i)%I_max),sngl(omega)
     .                       ,sngl(voltage/Rw)
     .                       ,sngl((motor_omega(i)/Kv)/Rw),sngl(uc(i))
...
c	   if (ip) write (6,100) 'prop acceleration',i,5, 
c     .               0.d0, 0.d0, 0.d0
c     .               ,Mx, My, Mz
...
c	if (ip) write (6,100) 'grav',0, 7
c     .                         , grav*xo(13)*aircraft%mass
c     .                         , grav*xo(16)*aircraft%mass
c     .                         , grav*xo(19)*aircraft%mass
c
c        if (ip) write (6,*) ' grav ',grav,xo(13),xo(16),xo(19)
c     .             ,aircraft%mass  ! jdw debug 4/29/22
c        write (6,*)' time,xo ',time,xo(13),xo(16),xo(19)
c     .             ,sqrt(xo(13)**2+xo(16)**2+xo(19)**2)
c        write (6,*) ' time,q ',time,q0,q1,q2,q3
c     .             ,sqrt(q0**2+q1**2+q2**2+q3**2)  jdw debug end of April 2022
...
c       We still see drift, when vehicles tumble and there are fast
c       changes.  I was able to compute the exponential to give an exact
c       no-drift solution.  We will not longer be
c       using qdot to update the quaternion.  JDW  5/17/22
c       (see the flight subroutine)
...
c	Still deciding whether I want it positive or negative.  Positive right now.
	...
	xo(29) = Power_tot
	xo(29 + 4*aircraft%num_propellers + 1) = Power_tot_BV

c	if (ip) write (6,100) 'Final Sum',0,8,
c     .                Fx_tot + grav*xo(13)*aircraft%mass
c     .              , Fy_tot + grav*xo(16)*aircraft%mass
c     .              , Fz_tot + grav*xo(19)*aircraft%mass
c     .               ,Mx_tot, My_tot, Mz_tot
c 100    format(' Fx Fy Fz Mx My Mz ',a30,2i3,6f12.3)
 100    format(' Fx Fy Fz Mx My Mz ',a30,2i3,6f15.7)

c	if (ip) write (6,49) (i,xo(i),
c     .       i = 29,29+2*aircraft%num_propellers+aircraft%num_batteries)
c 49	format(' xo ',6(i3,f10.3))
c
...
c	      write (6,*) ' Alin = '
c	      do i = 1, 12
c	         write (6,1000) (Alin(i,k),k=1,12)
c	      enddo
c	      write (6,*) ' Blin = '
c	      do i= 1, 12
c	         write (6,1000) (Blin(i,k),k=1,udim)
c	      enddo
c	   endif
 1000	   format(12e12.5)
